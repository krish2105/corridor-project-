...
| The scheme's key movement was **never counted** | JDA is converting this road to signal-free operation with seven U-turn bays. No U-turn column exists anywhere in the survey. The drawing shows 6 median gaps wide enough to turn through |
...
uv run python src/junction_books.py # -> out/junction_books/: 12 turning-movement sheets per junction
...
- `src/` — 48 modules. `tmc_parse` and `audit` are the core; `atlas`, `medians` and `dxf_inventory` read the CAD survey; `capacity`, `scheme_test`, `delay` and `economics` carry the findings.
